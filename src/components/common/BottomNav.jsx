    ...
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 pointer-events-none">
      ...
      {cartItems.length > 0 && (
        <div className="px-5 pb-2 animate-in slide-in-from-bottom-4 duration-300 pointer-events-auto">
          <Link
            to="/cart"
            className="cart-fab-green flex items-center justify-between mx-auto max-w-[80%] px-4 py-2 rounded-2xl text-white transition-transform active:scale-95 shadow-xl"
          >
            ...
            return (
              ...
              >
                <div className={`relative flex items-center justify-center w-10 h-8 rounded-2xl transition-all duration-300 ${
                  isActive ? 'bg-gradient-to-br from-[#4b1028] to-[#7d2949] shadow-md shadow-[#681f3b]/30' : ''
                }`}>
                ...
                <span className={`text-[9px] font-bold tracking-wide transition-all duration-300 ${isActive ? 'text-[#681f3b]' : 'text-stone-400'}`}>
                  {item.name}
                </span>
              </button>
            );
